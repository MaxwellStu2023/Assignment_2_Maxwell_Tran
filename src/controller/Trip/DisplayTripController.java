...
public class DisplayTripController extends Controller<Trip> {
    ...
    private void handleViewIndividual(ActionEvent event) {
        if (getCurrent() != null) {
            try {
                if (getCurrent().equals(model.getDestinations())) {
                    try {
                ...
                } else if (getCurrent().equals(model.getFlights())) {
                    try {
                        System.out.println("Hello World");
                ...
                ViewLoader.showErrorWindow(new ErrorModel(e, "You can't display Flights and Destination together!"));
            }
        }
    }
}
